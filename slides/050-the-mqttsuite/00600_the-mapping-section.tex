...
\begin{frame}[fragile]{The \alert{JSON Structure} of the \texttt{<\alert{mapping-section}>}s}
  ...
  \begin{itemize}
    ...
    \begin{description}[\texttt{static}]
      ...
      \item[\texttt{value}] is used if the incoming message should be \alert{interpreted as a value}. The actual mapping is done controlled by a \alert{\href{https://github.com/pantor/inja}{inja}} template, whereas the message is accessible via the template variable \texttt{\alert{message}}.
