\subsection{Architecture of the Echo Application}

\begin{frame}{Architecture of the Echo Application}
  \begin{center}
    \begin{figure}
      \par\bigskip
      \begin{tikzpicture}
...
%%% Start Lowlevel Server/Client
        \draw (em.north) node[anchor=south east, outer sep=0, draw, fill=red!30, minimum width=5cm, minimum height=1cm] (llst) {\texttt{SocketServer<SCF\footnote{\label{scf2e}\texttt{SCF} \ldots \texttt{SocketContextFactory}}>}};
        \draw (em.north) node[anchor=south west, outer sep=0, draw, fill=red!30, minimum width=5cm, minimum height=1cm] (llct) {\texttt{SocketClient<SCF\textsuperscript{\ref{scf2e}}>}};
%% End Lowlevel Server/Client
%% Start SocketConnection
        \draw (llst.north east) node[anchor=south, draw, fill=orange!30, inner sep=0, outer sep=0, minimum width=10cm, minimum height=1cm] (sc) {\texttt{SocketConnection<SCF\textsuperscript{\ref{scf2e}}>}};
...
%% SocketContexts
      ...
      \end{tikzpicture}
      \caption{Architecture of the Echo application.}
    \end{figure}
  \end{center}
\end{frame}
