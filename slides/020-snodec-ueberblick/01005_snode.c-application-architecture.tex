\subsection{Architecture of a Typical SNode.C-Application}

\begin{frame}{Architecture of a Typical SNode.C-Application}
  \begin{center}
    \begin{figure}
      \par\bigskip
      \begin{tikzpicture}
...
%%% End draw dashed line above Eventloop-Layer
%% SocketContexts
        \draw (sc.north) node[anchor=south east, outer sep=0, inner sep=0, minimum width=5cm, minimum height=1cm, draw, fill=yellow!30] {\texttt{ServerContext}};
        \draw (sc.north) node[anchor=south west, outer sep=0, inner sep=0, minimum width=5cm, minimum height=1cm, draw, fill=yellow!30] {\texttt{ClientContext}};
      \end{tikzpicture}
      \caption{Typical Architecture of a SNode.C-Application.}
    \end{figure}
  \end{center}
\end{frame}
